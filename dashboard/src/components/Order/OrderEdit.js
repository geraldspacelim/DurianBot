...
const OrderEdit = () => {
  ...
  useEffect(() => {
    axios
      .get(`http://159.223.69.13:8080/api/v1/order/getOrder/${id}`)
      .then((res) => {
        ...
        axios.get(`http://159.223.69.13:8080/api/v1/shop/`).then((res) => {
  ...
  const submitChanges = (e) => {
    ...
    if (window.confirm("Do you want to submit changes?")) {
      ...
      axios
        .post(`http://159.223.69.13:8080/api/v1/order/updateOrder/${id}`, data)
        .then((res) => {
          setIsLoading(false);
          history.push("/");
        })
              ...
              <div className="col">
                ...
                >
                  {deliveryOptions.map((option, idx) => (
                    <option key={idx}>{option.mode}</option>
                  ))}
                </select>
              </div>
            </div>
          ...
          <div className="form-group">
            <div className="row">
              ...
              <div className="col-sm-4">
                ...
                >
                  {promos.map((option, idx) => (
                    <option key={idx}>{option.code}</option>
                  ))}
                </select>
              </div>
            </div>
