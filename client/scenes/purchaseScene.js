...
step2.on("text", (ctx) => {
  const packageObj = ctx.wizard.state.packages.find(
    (o) => o.name === ctx.message.text
  );
  if (packageObj) {
    ...
    ctx.reply("Please choose a weight (g)", {
      // Package Size Message
      reply_markup: {
...
step3.on("text", (ctx) => {
  const package = ctx.wizard.state.selectedPackage.details.find(
    (o) => o.size === ctx.message.text
  );
  if (package) {
...
step4.hears(QTY_TYPES, (ctx) => {
  if (ctx.match !== "more") {
    ...
    ctx.reply("How many do you want ?"); // Select Custom Quantity Message
...
step5.on("text", (ctx) => {
  ...
  ctx.reply("Please choose a delivery option", {
    // Select Delivery Option Message
    reply_markup: {
      keyboard: keyboardGroup,
      resize_keyboard: true,
      one_time_keyboard: true,
...
step10.hears(RADIO_TYPES, async (ctx) => {
  if (ctx.match === "Yes") {
    ctx.reply("Do you have a promo code ?"); // Promo Code Message
    return ctx.wizard.next();
  }
...
step11.on("text", async (ctx) => {
  const promo = ctx.wizard.state.promos.find(
    (o) => o.code === ctx.message.text
  );
  if (promo) {
    const is_promo = true;
...
step12.hears(RADIO_TYPES, (ctx) => {
  ...
  ctx.reply(
    `Thank you ${ctx.wizard.state.name} for using Smelly Story Bot! Please contact @smellystory if you have any queries!
    `
  ); // Thank you Message
...
step14.hears(RADIO_TYPES, (ctx) => {
  ...
  ctx.reply("What is your name?"); // Name Message
